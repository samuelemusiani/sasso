...
const logs = ref([
  { id: 1, timestamp: '2025-09-25 18:45:23', type: 'info', message: 'VM #1001 avviata con successo', user: 'admin' },
  { id: 2, timestamp: '2025-09-25 18:44:15', type: 'warning', message: 'Utilizzo CPU al 85% per VM #1002', user: 'user1' },
  { id: 3, timestamp: '2025-09-25 18:43:01', type: 'error', message: 'Errore nella creazione della rete "test-net"', user: 'user2' },
  { id: 4, timestamp: '2025-09-25 18:42:33', type: 'info', message: 'Nuova SSH key aggiunta per utente "dev-team"', user: 'admin' },
  { id: 5, timestamp: '2025-09-25 18:41:45', type: 'success', message: 'Backup VM #1003 completato', user: 'system' },
  { id: 6, timestamp: '2025-09-25 18:40:12', type: 'warning', message: 'Spazio disco insufficiente per VM #1004', user: 'user3' },
  { id: 7, timestamp: '2025-09-25 18:39:58', type: 'info', message: 'Aggiornamento configurazione rete completato', user: 'admin' },
  { id: 8, timestamp: '2025-09-25 18:38:22', type: 'error', message: 'Connessione VPN interrotta per utente "mobile-user"', user: 'system' },
])
...
const metrics = computed(() => {
  ...
  const cpuPercentage = user.value.max_cores > 0 ? Math.min(100, Math.round((usedCores / user.value.max_cores) * 100)) : 0
  const ramPercentage = user.value.max_ram > 0 ? Math.min(100, Math.round((usedRam / user.value.max_ram) * 100)) : 0
  const diskPercentage = user.value.max_disk > 0 ? Math.min(100, Math.round((usedDisk / user.value.max_disk) * 100)) : 0
  const netPercentage = user.value.max_nets > 0 ? Math.min(100, Math.round((usedNets / user.value.max_nets) * 100)) : 0
...
<template>
  ...
  <div class="h-full overflow-auto">
    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8 px-2">
      <div
        ...
        class="card shadow-2xl border border-white/30 
               ...
               hover:bg-gradient-to-br hover:from-white/20 hover:to-white/12
          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2 w-40">
              <div class="btn btn-square btn-md rounded-xl btn-primary p-0 m-1 flex-shrink-0">
                <Icon :icon="metric.icon" class="text-xl" />
              </div>
              <div class="min-w-0 flex-1">
                <h3 class="font-bold text-lg text-base-content truncate drop-shadow-sm mb-1">{{ metric.title }}</h3>
                <span class="text-sm font-semibold text-base-content/80 drop-shadow-sm truncate block">
                ...
                <circle
                  ...
                  :style="{
                    filter: `drop-shadow(0 0 4px ${
                      metric.percentage < 60 ? 'rgb(34 197 94 / 0.6)' :
                      metric.percentage < 80 ? 'rgb(245 158 11 / 0.6)' :
                      'rgb(239 68 68 / 0.6)'
                    })`
                  }"
                />
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 px-2">
      <div class="card shadow-2xl border border-white/30 
                  bg-gradient-to-br from-white/15 to-white/8 
                  backdrop-blur-2xl backdrop-saturate-200
                  hover:shadow-[0_25px_50px_-12px_rgba(0,0,0,0.25),0_0_20px_rgba(255,255,255,0.1)]
                  hover:scale-[1.01] hover:border-white/40
        ...
        <div class="card-body relative z-10">
          <h2 class="card-title text-base-content drop-shadow-sm">
            <Icon icon="material-symbols:history" class="text-xl drop-shadow-sm" />
            Attività Recente
          </h2>
          <p class="text-base-content/70 text-sm">Le ultime operazioni sulle VM verranno mostrate qui.</p>
          <div class="mt-4 space-y-2">
            <div v-for="vm in vms.slice(0, 3)" :key="vm.id" 
                 class="flex items-center gap-3 p-2 rounded-lg bg-white/10 backdrop-blur-sm 
                        border border-white/20 min-w-0 shadow-sm">
      ...
      <div class="card shadow-2xl border border-white/30 
        ...
        <div class="card-body relative z-10">
          <h2 class="card-title text-base-content drop-shadow-sm">
            <Icon icon="material-symbols:analytics" class="text-xl drop-shadow-sm" />
            Statistiche Rapide
          </h2>
          <div class="mt-4 space-y-3">
            <div class="stat bg-white/10 backdrop-blur-sm border border-white/20 rounded-lg p-3 shadow-sm">
      ...
      <div class="card shadow-2xl border border-white/30 
        ...
        <div class="card-body relative z-10">
          <h2 class="card-title text-base-content drop-shadow-sm mb-4">
            <Icon icon="material-symbols:terminal" class="text-xl drop-shadow-sm" />
            Log Sistema
          ...
          <div class="max-h-96 overflow-y-auto pr-2 custom-scrollbar">
            <div class="space-y-2">
              <div v-for="log in logs" :key="log.id" 
                   class="flex items-start gap-3 p-3 rounded-lg bg-white/10 backdrop-blur-sm 
                          border border-white/20 shadow-sm hover:bg-white/15 
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center justify-between mb-1">
                    <span class="text-xs font-mono text-base-content/60 bg-base-300/30 px-2 py-1 rounded">
                      {{ log.timestamp }}
                    </span>
