...
function getStatusIcon(status: string) {
  switch (status) {
    case 'active': return 'material-symbols:check-circle'
    case 'inactive': return 'material-symbols:cancel'
  ...
  <div class="space-y-8">
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-base-content/70 text-lg">Gestisci le connessioni di rete per {{ vm?.name || `VM #${vmid}` }}</p>
        </div>
      ...
      <div class="liquid-glass-card-no-scale bg-gradient-to-br from-primary/5 via-transparent to-info/5">
        <div class="p-6">
          <button v-if="!showAddForm && !editingInterface" 
          ...
          <div v-if="showAddForm || editingInterface" class="space-y-4">
            <div class="flex items-center gap-3 mb-4">
              <Icon :icon="editingInterface ? 'material-symbols:edit' : 'material-symbols:add-circle'" 
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="liquid-glass-card-no-scale bg-gradient-to-br from-warning/10 via-transparent to-warning/5 p-6">
          <div class="flex items-start gap-4">
            <Icon icon="material-symbols:warning" class="text-2xl text-warning flex-shrink-0 mt-1" />
            <div>
              <h3 class="font-bold text-base-content mb-2">Attenzione</h3>
              <p class="text-base-content/80 text-sm leading-relaxed">
                È possibile aggiungere interfacce mentre la VM è in esecuzione. La VM vedrà l'interfaccia, 
                ma non sarà configurata automaticamente. Per configurare l'interfaccia sarà necessario riavviare la VM.
              </p>
            </div>
          </div>
        </div>

        <div class="liquid-glass-card-no-scale bg-gradient-to-br from-info/10 via-transparent to-info/5 p-6">
          <div class="flex items-start gap-4">
            <Icon icon="material-symbols:info" class="text-2xl text-info flex-shrink-0 mt-1" />
            <div>
              <h3 class="font-bold text-base-content mb-2">Informazioni VLAN</h3>
              <p class="text-base-content/80 text-sm leading-relaxed">
                Il tag VLAN è opzionale (usa 0 se non sai cosa inserire). Serve per separare diverse VM a livello 2. 
                Le interfacce con lo stesso tag VLAN possono comunicare tra loro. Il gateway è sulla VLAN 0 (untagged).
              </p>
            </div>
          </div>
        </div>
      ...
      <div>
        ...
        <div v-if="interfaces.length === 0" class="liquid-glass-card-no-scale p-8 text-center">
          <Icon icon="material-symbols:network-off" class="text-6xl text-base-content/30 mb-4" />
          <h3 class="text-lg font-semibold text-base-content/70 mb-2">Nessuna interfaccia configurata</h3>
              ...
              <div class="flex items-center gap-6">
                ...
                <div class="flex items-center gap-3">
                  <div class="btn btn-square btn-md rounded-xl p-0 m-1 flex-shrink-0"
                    ...
                    <Icon :icon="getStatusIcon(iface.status)" class="text-xl" />
                  </div>
                  <div>
                    <h3 class="font-bold text-lg text-base-content">
                      Interfaccia #{{ iface.id }}
                    </h3>
                    <span class="text-sm font-semibold capitalize" :class="getStatusColor(iface.status)">
                      {{ iface.status }}
                    </span>
                  </div>
                </div>
                ...
                <div class="hidden lg:flex items-center gap-6">
                  <div class="text-center">
                    ...
                    <p class="text-base font-bold text-base-content">{{ netMap.get(iface.vnet_id) || 'N/A' }}</p>
                  </div>
                  <div class="text-center">
            ...
            <div class="lg:hidden mt-4 pt-4 border-t border-white/20">
              <div class="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <span class="font-semibold text-base-content">{{ netMap.get(iface.vnet_id) || 'N/A' }}</span>
                </div>
                <div>
                ...
                <div>
                  ...
                  <span class="font-mono text-xs text-base-content">{{ iface.gateway || 'N/A' }}</span>
                </div>
              </div>
            </div>
          </div>
