  ...
  <div class="h-full overflow-auto">
    ...
    <div class="mb-8 px-2">
      <div class="flex items-center gap-3 mb-4">
        <div class="btn btn-square btn-lg rounded-xl btn-primary p-0 flex-shrink-0">
          <IconifyIcon icon="material-symbols:admin-panel-settings" class="text-2xl" />
        </div>
        <div>
          <h1 class="text-3xl font-bold text-base-content">Pannello Amministrazione</h1>
          <p class="text-base-content/70">
            Gestisci utenti, domini di autenticazione e configurazioni globali
          </p>
        </div>
      </div>
    </div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8 px-2">
      ...
      >
        <div class="card-body p-6">
          ...
          <div class="flex items-start justify-between mb-4">
            <div class="flex items-center gap-3">
              ...
              >
                <IconifyIcon :icon="section.icon" class="text-2xl" :class="section.iconColor" />
              </div>
              <div>
          ...
          <div class="flex items-center justify-between mt-auto">
            ...
            >
              <IconifyIcon icon="material-symbols:arrow-forward" class="text-lg" />
            </div>
          </div>
        </div>
        ...
        <!-- Indicatore hover -->
        <div
          class="absolute bottom-0 left-0 w-full h-1 bg-gradient-to-r opacity-0 group-hover:opacity-100 transition-opacity duration-300"
          :class="
            section.iconColor.replace('text-', 'from-').replace('-500', '-400') + ' to-transparent'
          "
        ></div>
      </RouterLink>
    </div>
    ...
    <div class="px-2 mb-6">
      <h2 class="text-xl font-bold text-base-content mb-4 flex items-center gap-2">
        <IconifyIcon icon="material-symbols:flash-on" class="text-warning" />
        Azioni Rapide
      </h2>

      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <RouterLink
          to="/admin/realms?add=true"
          class="btn btn-outline btn-sm flex-col gap-2 p-4 h-auto hover:bg-purple-500/10"
        >
          <IconifyIcon icon="material-symbols:add-circle" class="text-2xl text-purple-500" />
        ...
        >
          <IconifyIcon icon="material-symbols:vpn-key" class="text-2xl text-emerald-500" />
          <span class="text-xs">Aggiungi Chiave</span>
        </RouterLink>

        <RouterLink
          to="/admin/port-forwards?create=true"
          class="btn btn-outline btn-sm flex-col gap-2 p-4 h-auto hover:bg-orange-500/10"
        >
          <IconifyIcon icon="material-symbols:add-box" class="text-2xl text-orange-500" />
