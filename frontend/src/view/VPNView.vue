...
const maskedConfig = computed(() => {
  ...
  return vpnConfig.value
    .replace(/(PrivateKey\s*=\s*)([A-Za-z0-9+/=]+)/g, '$1' + '•'.repeat(35))
    .replace(/(PresharedKey\s*=\s*)([A-Za-z0-9+/=]+)/g, '$1' + '•'.repeat(35))
})
  ...
  <div class="space-y-8">
    ...
    <div class="text-center">
      <h1 class="text-4xl font-bold text-base-content mb-2">
        <IconifyIcon icon="material-symbols:vpn-key" class="inline mr-3 text-primary" />
        Configurazione VPN
      </h1>
      ...
      <div class="card-body p-6">
        <div class="flex items-center justify-between mb-6">
          <h2 class="card-title text-base-content flex items-center gap-3">
            <IconifyIcon icon="material-symbols:settings" class="text-primary text-2xl" />
          ...
          <div class="flex items-center gap-2">
            ...
            <button @click="downloadConfig()" class="btn btn-primary btn-sm gap-2">
              <IconifyIcon icon="material-symbols:download" class="text-lg" />
              Download .conf
            </button>
          </div>
        </div>
          ...
          <div class="flex items-center justify-between mb-2">
            ...
            <div class="flex items-center gap-2">
              <div class="badge badge-warning badge-sm">
                <IconifyIcon icon="material-symbols:visibility-off" class="text-xs mr-1" />
                Chiavi Mascherate
              </div>
              <div class="badge badge-outline badge-sm">
                <IconifyIcon icon="material-symbols:code" class="text-xs mr-1" />
                {{ vpnStats.lines }} linee
              </div>
            </div>
          </div>
          <pre
            class="text-sm font-mono text-base-content whitespace-pre-wrap break-all leading-relaxed max-h-96 overflow-y-auto"
            >{{ maskedConfig }}</pre
          >
        </div>
        ...
        <div class="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div class="bg-info/10 rounded-lg p-4 border border-info/20">
            <h3 class="font-semibold text-info mb-2 flex items-center gap-2">
              <IconifyIcon icon="material-symbols:info" class="text-lg" />
              Come utilizzare
            </h3>
          ...
          <div class="bg-success/10 rounded-lg p-4 border border-success/20">
            <h3 class="font-semibold text-success mb-2 flex items-center gap-2">
              <IconifyIcon icon="material-symbols:content-copy" class="text-lg" />
              Copia e Download
            </h3>
            <ul class="text-sm text-base-content/80 space-y-1">
          ...
          <div class="bg-warning/10 rounded-lg p-4 border border-warning/20">
            <h3 class="font-semibold text-warning mb-2 flex items-center gap-2">
              <IconifyIcon icon="material-symbols:security" class="text-lg" />
              Sicurezza
            </h3>
            <ul class="text-sm text-base-content/80 space-y-1">
    ...
    <div v-else class="text-center py-12">
      <div class="liquid-glass-card p-8">
        ...
        <h3 class="text-xl font-semibold text-base-content mb-2">
          Configurazione VPN Non Disponibile
        </h3>
        <p class="text-base-content/70">
          La configurazione VPN non è ancora stata generata o non è disponibile.
        </p>
        <button @click="fetchVPNConfig()" class="btn btn-primary mt-4 gap-2">
          <IconifyIcon icon="material-symbols:refresh" class="text-lg" />
          Riprova
        </button>
      </div>
    </div>
