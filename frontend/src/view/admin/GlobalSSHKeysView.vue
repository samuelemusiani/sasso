...
const keyStats = computed(() => ({
  ...
  otherKeys: keys.value.filter(key => 
    !key.key.startsWith('ssh-rsa') && 
    !key.key.startsWith('ssh-ed25519')
  ).length
}))
...
function getKeyIcon(key: string): string {
  ...
  switch (type) {
    case 'RSA': return 'material-symbols:key'
    case 'Ed25519': return 'material-symbols:enhanced-encryption'
    case 'ECDSA': return 'material-symbols:security'
    default: return 'material-symbols:vpn-key'
  }
}
...
function getKeyColor(key: string): string {
  const type = getKeyType(key)
  switch (type) {
    case 'RSA': return 'text-blue-500'
    case 'Ed25519': return 'text-green-500'
    case 'ECDSA': return 'text-purple-500'
  ...
  <div class="min-h-screen">
    <div class="container mx-auto px-4 py-8">
      ...
      <div class="bg-base-100/80 backdrop-blur-sm border border-base-300/50 rounded-2xl p-6 mb-6 shadow-xl">
        <div class="flex items-center gap-4 mb-4">
          <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center shadow-lg">
            <Icon icon="material-symbols:vpn-key" class="text-2xl text-white" />
          </div>
          <div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 px-2">
        <div class="stat bg-gradient-to-br from-primary/10 to-primary/20 border border-primary/20 rounded-xl">
          <div class="stat-figure text-primary">
            <Icon icon="material-symbols:vpn-key" class="text-3xl" />
          </div>
        ...
        <div class="stat bg-gradient-to-br from-blue-500/10 to-blue-600/10 border border-blue-500/20 rounded-xl">
          <div class="stat-figure text-blue-500">
            <Icon icon="material-symbols:key" class="text-3xl" />
          </div>
        ...
        <div class="stat bg-gradient-to-br from-green-500/10 to-green-600/10 border border-green-500/20 rounded-xl">
          <div class="stat-figure text-green-500">
            <Icon icon="material-symbols:enhanced-encryption" class="text-3xl" />
          </div>
        ...
        <div class="stat bg-gradient-to-br from-purple-500/10 to-purple-600/10 border border-purple-500/20 rounded-xl">
          <div class="stat-figure text-purple-500">
            <Icon icon="material-symbols:security" class="text-3xl" />
          </div>
      ...
      <div v-if="addingKey" class="mb-6 px-2">
        <div class="bg-base-100/80 backdrop-blur-sm border border-base-300/50 rounded-2xl p-6 shadow-lg">
          <div class="flex items-center gap-3 mb-6">
            <div class="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center">
              <Icon icon="material-symbols:add" class="text-xl text-primary" />
            </div>
            <div>
              <h3 class="font-bold text-xl">Aggiungi Nuova Chiave SSH</h3>
              <p class="text-sm text-base-content/70">Inserisci i dettagli della nuova chiave SSH globale</p>
            </div>
          </div>
          ...
          <form @submit.prevent="addKey" class="space-y-4">
            ...
            <div class="form-control">
              <label class="label">
                ...
                <span class="label-text-alt text-base-content/60">Incolla la chiave pubblica SSH</span>
              </label>
              <textarea
                v-model="newKey.key"
      ...
      <div class="px-2">
        <div class="bg-base-100/80 backdrop-blur-sm border border-base-300/50 rounded-2xl shadow-lg overflow-hidden">
            ...
            <table class="table table-zebra w-full">
              <thead>
                <tr class="border-base-300">
                  ...
                  <th class="bg-base-200/50">
                    <div class="flex items-center gap-2">
                      <Icon icon="material-symbols:vpn-key" class="text-sm" />
                      Nome & Tipo
                    </div>
                  </th>
                  <th class="bg-base-200/50">
                  ...
                  <tr class="hover">
                    ...
                    <td>
                      <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-full flex items-center justify-center"
                             :class="getKeyColor(key.key).replace('text-', 'bg-').replace('-500', '-100')">
                          <Icon :icon="getKeyIcon(key.key)" :class="getKeyColor(key.key)" class="text-xl" />
                        </div>
                        <div>
                    ...
                    <td>
                      <div class="flex gap-2 justify-center">
                        ...
                        >
                          <Icon :icon="expandedKey === key.id ? 'material-symbols:expand-less' : 'material-symbols:visibility'" />
                        ...
                        <div class="bg-base-100 rounded-xl p-6 shadow-sm border border-base-300">
                          <div class="flex items-center gap-3 mb-6">
                            ...
                            <div>
                              ...
                              <p class="text-sm text-base-content/70">Chiave SSH {{ getKeyType(key.key) }}</p>
                            </div>
                          </div>
                          ...
                          <div class="space-y-4">
                            <div class="form-control">
                              <label class="label">
                                <span class="label-text font-medium text-base-content">Chiave SSH Completa</span>
                                <button 
          ...
          <div v-else-if="keys.length === 0" class="p-8 text-center">
            <Icon icon="material-symbols:vpn-key-off" class="text-6xl text-base-content/30 mb-4" />
            <p class="text-lg font-medium text-base-content/70 mb-2">Nessuna chiave SSH configurata</p>
            <p class="text-base-content/50 mb-4">Aggiungi la prima chiave SSH globale per iniziare</p>
