    ...
    .then(async (response) => {
      ...
      if (newPortForwardId) {
        ...
        const latestPf = portForwards.value
          .filter(
            (pf) =>
              pf.dest_ip === newPortForward.value.dest_ip &&
              pf.dest_port === destPort &&
              !pf.approved,
          )
    ...
    <div class="mb-6 px-2">
      ...
      <div class="flex items-center gap-3 mb-4">
        <div class="btn btn-square btn-lg rounded-xl btn-primary p-0 flex-shrink-0">
          <IconifyIcon icon="material-symbols:router" class="text-2xl" />
        </div>
        <div>
          ...
          <div class="card-body">
            ...
            <div class="flex flex-col md:flex-row gap-4 items-center justify-between">
              ...
              <div class="flex gap-2 shrink-0">
                ...
                >
                  <IconifyIcon :icon="status.icon" class="text-sm" :class="status.color" />
                  {{ status.label }}
                </button>
          ...
          <div class="card-body">
            <div class="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p class="text-sm text-base-content/70">
                  Crea una nuova regola di port forwarding (sarà automaticamente approvata)
                </p>
              </div>
            </div>
            ...
            <form @submit.prevent="createPortForward" class="space-y-4">
              ...
              <div class="flex justify-end gap-2 pt-4">
                <button type="button" @click="addingPortForward = false" class="btn btn-ghost">
                  Annulla
                </button>
                <button type="submit" :disabled="isProcessing" class="btn btn-primary gap-2">
                  ...
                  <IconifyIcon v-else icon="material-symbols:add" />
            ...
            <div class="overflow-x-auto">
              ...
              <table v-else-if="filteredPortForwards.length > 0" class="table table-zebra w-full">
                ...
                <tbody>
                  <tr v-for="pf in filteredPortForwards" :key="pf.id" class="hover">
                    ...
                    <td>
                      <div class="font-mono text-sm font-medium">:{{ pf.out_port }}</div>
                    </td>
                    <td>
                      <div class="font-mono text-sm">{{ pf.dest_ip }}:{{ pf.dest_port }}</div>
                    </td>
                    <td>
                      <div class="badge gap-1" :class="getApprovalBadge(pf.approved).class">
                        <IconifyIcon :icon="getApprovalBadge(pf.approved).icon" class="text-xs" />
                    ...
                    <td>
                      <div class="flex gap-2">
                        <button
                          v-if="!pf.approved"
                          @click="approvePortForward(pf.id)"
                          class="btn btn-success btn-sm gap-1"
                        >
                          <IconifyIcon icon="material-symbols:check" /> Approva
                        </button>
                        <button
              ...
              <div v-else class="p-8 text-center">
                ...
                <p class="text-lg font-medium text-base-content/70 mb-2">
                  Nessun port forwarding trovato
                </p>
                <p class="text-base-content/50">
                  Utilizza il pulsante "Nuovo Port Forward" per crearne uno
                </p>
              </div>
            </div>
          </div>
