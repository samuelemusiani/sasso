  ...
  <div class="flex flex-col gap-2 overflow-y-auto">
    ...
    <CreateNew title="New VM" :create="createVM" :error="error">
      <div>
        ...
        <input required type="text" id="name" v-model="name" class="input w-full border p-2 rounded-lg w-24" />
      </div>
      <div>
        ...
        <tbody class="divide-y">
          <tr v-for="vm in vms" :key="vm.id">
            <td class="flex flex-col">
              ...
              <!-- TODO: new fancy notes -->
            </td>
            <td class="">{{ vm.cores }}</td>
            <td class="">{{ vm.ram }}</td>
            <td class="">{{ vm.disk }}</td>
            <td class="capitalize font-semibold" :class="getStatusClass(vm.status)">{{ vm.status }}</td>
