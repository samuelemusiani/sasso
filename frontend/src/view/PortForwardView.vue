<template>
  <div class="min-h-screen">
    <div class="container mx-auto px-4 py-8">
      <!-- Header -->
      <div
        class="bg-base-100/80 backdrop-blur-sm border border-base-300/50 rounded-2xl p-6 mb-6 shadow-xl liquid-glass-card-no-scale bg-gradient-to-br from-primary/5 via-transparent to-accent/5"
      >
        <div class="flex items-center justify-between mb-4">
          ...
          >
            <IconifyIcon icon="material-symbols:add" class="text-lg" />
            Nuova Richiesta
          </button>
        </div>
      </div>
      ...
      <div
        v-if="showCreateForm"
        class="backdrop-blur-sm border border-base-300/50 rounded-2xl p-6 mb-6 shadow-xl liquid-glass-card-no-scale bg-gradient-to-br from-primary/5 via-transparent to-accent/5"
      >
        <h2 class="text-xl font-bold mb-4 flex items-center gap-2">
          <IconifyIcon icon="material-symbols:add-box" class="text-primary" />
        ...
        <form @submit.prevent="submitPortForwardRequest" class="space-y-4">
          ...
          <div class="form-control">
            ...
            <label class="label">
              <span class="label-text-alt text-base-content/60"
                >Indirizzo IP del dispositivo di destinazione</span
              >
            </label>
          </div>
          ...
          <div class="form-control">
            ...
            <label class="label">
              <span class="label-text-alt text-base-content/60"
                >Porta del servizio sul dispositivo destinazione</span
              >
            </label>
          </div>
          ...
          <div class="flex gap-2 justify-end">
            <button type="button" @click="showCreateForm = false" class="btn btn-ghost">
              Annulla
            </button>
            <button type="submit" :disabled="isSubmitting" class="btn btn-primary gap-2">
              <span v-if="isSubmitting" class="loading loading-spinner loading-sm"></span>
              <IconifyIcon v-else icon="material-symbols:send" />
      ...
      <!-- Lista Port Forwards -->
      <div
        class="backdrop-blur-sm border border-base-300/50 rounded-2xl shadow-xl overflow-hidden liquid-glass-card-no-scale bg-gradient-to-br from-primary/5 via-transparent to-accent/5"
      >
        <div class="p-6 border-b border-base-300/50">
          <h2 class="text-xl font-bold flex items-center gap-2">
            <IconifyIcon icon="material-symbols:list" class="text-primary" />
            Le Tue Richieste
          </h2>
        </div>
        ...
        <div class="overflow-x-auto">
          ...
          <table v-else-if="portForwards.length > 0" class="table table-zebra w-full">
            ...
            <tbody>
              <tr v-for="portForward in portForwards" :key="portForward.id" class="hover">
                ...
                <td>
                  <div class="font-mono text-sm font-medium">:{{ portForward.out_port }}</div>
                </td>
                <td>
                ...
                <td>
                  <div class="badge gap-1" :class="getApprovalBadge(portForward.approved).class">
                    <IconifyIcon
                      :icon="getApprovalBadge(portForward.approved).icon"
                      class="text-xs"
                    />
                ...
                <td>
                  <div class="flex gap-2">
                    ...
                    >
                      <IconifyIcon icon="material-symbols:delete" class="text-sm" />
                    </button>
                  </div>
                </td>
              </tr>
          ...
          <div v-else class="p-8 text-center">
            ...
            <button @click="showCreateForm = true" class="btn btn-primary gap-2">
...
function deletePortForward(portForward: PortForward) {
  if (
    !confirm(
      `Sei sicuro di voler eliminare il port forwarding per ${portForward.dest_ip}:${portForward.dest_port}?`,
    )
  ) {
    return
  }
