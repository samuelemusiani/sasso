...
<template>
  <div class="flex flex-col gap-4">
    ...
    <div class="flex flex-col gap-2">
      ...
      <p>
        ...
        <ul class="list-disc pl-6">
          <li>
            <b :class="getStatusClass('pre-creating')">Pre-Creating</b>: the VM is waiting to be created.
          </li>
          <li>
            <b :class="getStatusClass('creating')">Creating</b>: the VM is being created. This process can take a
            while, be patient!
          </li>
          <li>
          ...
          <li>
            <b :class="getStatusClass('stopped')">Stopped</b>: the VM is created but not running. You can start it
            by clicking the "Start" button on the right of it.
          </li>
          <li>
            <b :class="getStatusClass('running')">Running</b>: the VM is running and you can connect to it.
          </li>
          <li>
          ...
          <li>
            <b :class="getStatusClass('deleting')">Deleting</b>: the VM is being deleted.
          </li>
          <li>
            <b :class="getStatusClass('unknown')">Unknown</b>: the VM is in an unknown state.
    ...
    <div class="flex flex-col gap-2">
      ...
      <p>
        You can perform various actions on your VMs:
        <ul class="list-disc pl-6">
          <li>
          ...
          <li>
            ...
            so it <b><u>can cause data loss</u></b> if the VM is not properly shut down. Use it only if the VM is
            not responding and you cannot stop it gracefully from the VM details page.
          </li>
          <li>
            <b class="text-info">Restart</b>: restarts a running VM. This is like pulling the power
            cord and plugging it back in, so it <b><u>can cause data loss</u></b> if the VM is not properly shut
            down. Use it only if the VM is not responding and you cannot restart it gracefully from
            the VM details page.
          </li>
          <li>
    ...
    <div class="flex flex-col gap-2">
      ...
      <p>
        To access the VM you first need to add an <RouterLink to="/ssh-keys" class="link
          link-primary">SSH key to your account</RouterLink>, active the <RouterLink to="/vpn"
          class="link link-primary">VPN</RouterLink> on your
      ...
      <p>
        The command to connect to the VM is usually something like this:
      </p>
      ...
      >
      <p>
        Please note that <b class="text-info">SSH Keys</b> only apply to "Stopped" VMs. If you add an SSH key to a VM
        that is already running, the key will not be added until the next boot.
      </p>
    </div>
  </div>
</template>
